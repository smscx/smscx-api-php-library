...
use Smscx\Client\Model\ModelInterface;
...
class ObjectSerializer
{
     ...
     * @param string $format the format of type of the data
     *
    ...
    public static function sanitizeForSerialization($data, $type = null, $format = null)
    {
        ...
        if (is_object($data)) {
            ...
            if ($data instanceof ModelInterface) {
                $formats = $data::clientFormats();
                foreach ($data::clientTypes() as $property => $clientType) {
                    $getter = $data::getters()[$property];
                    $value = $data->$getter();
                    if ($value !== null && !in_array($clientType, ['\DateTime', '\SplFileObject', 'array', 'bool', 'boolean', 'byte', 'double', 'float', 'int', 'integer', 'mixed', 'number', 'object', 'string', 'void'], true)) {
                        $callable = [$clientType, 'getAllowableEnumValues'];
                        if (is_callable($callable)) {
                            /** array $callable */
                            $allowedEnumTypes = $callable();
                            if (!in_array($value, $allowedEnumTypes, true)) {
                                $imploded = implode("', '", $allowedEnumTypes);
                                throw new \InvalidArgumentException("Invalid value for enum '$clientType', must be one of: '$imploded'");
                            }
                        }
                    }
                    if (($data::isNullable($property) && $data->isNullableSetToNull($property)) || $value !== null) {
                        $values[$data::attributeMap()[$property]] = self::sanitizeForSerialization($value, $clientType, $formats[$property]);
                    }
                }
            } else {
                foreach($data as $property => $value) {
     ...
     * @param string $clientType  ClientType eg. array or object
    ...
    public static function toQueryValue(
        ...
        if (
            empty($value)
            && ($value !== false || $clientType !== 'boolean') // if $value === false and $clientType ==='boolean' it isn't empty
        ) {
            if ($required) {
                return ["{$paramName}" => ''];
            } else {
        ...
        $query = [];
        $value = (in_array($clientType, ['object', 'array'], true)) ? (array)$value : $value;
        ...
        if ($clientType === 'object' && ($style === 'deepObject' || $explode)) {
            return $value;
        }

        if ('boolean' === $clientType && is_bool($value)) {
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if (method_exists($class, 'getAllowableEnumValues')) {
            ...
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\Smscx\Client\Model\\' . $data->{$discriminator};
            ...
            $instance = new $class();
            foreach ($instance::clientTypes() as $property => $type) {
