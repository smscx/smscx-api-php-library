...
/**
 * DataNumberLookupWebhook Class Doc Comment
 *
 * @category Class
 * @package  Smscx
 * @author   SMS Connexion
 ...
 */
class DataNumberLookupWebhook implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $clientModelName = 'DataNumberLookupWebhook';
    ...
    */
     ...
     * Sets lookup_bulk_id
     *
     * @param string $lookup_bulk_id Unique bulk lookup identifier
     *
     * @return self
     */
    public function setLookupBulkId($lookup_bulk_id)
    {
        if ((mb_strlen($lookup_bulk_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $lookup_bulk_id when calling DataNumberLookupWebhook., must be smaller than or equal to 36.');
        }
        if ((mb_strlen($lookup_bulk_id) < 36)) {
            throw new \InvalidArgumentException('invalid length for $lookup_bulk_id when calling DataNumberLookupWebhook., must be bigger than or equal to 36.');
        }
        if ((!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/", $lookup_bulk_id))) {
            throw new \InvalidArgumentException("invalid value for \$lookup_bulk_id when calling DataNumberLookupWebhook., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.");
        }


        if (is_null($lookup_bulk_id)) {
            array_push($this->clientNullablesSetToNull, 'lookup_bulk_id');
        } else {
            $nullablesSetToNull = $this->getClientNullablesSetToNull();
            $index = array_search('lookup_bulk_id', $nullablesSetToNull);
            if ($index !== FALSE) {
    ...
    public function setLookupId($lookup_id)
    {
        if ((mb_strlen($lookup_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $lookup_id when calling DataNumberLookupWebhook., must be smaller than or equal to 36.');
        }
        if ((mb_strlen($lookup_id) < 36)) {
            throw new \InvalidArgumentException('invalid length for $lookup_id when calling DataNumberLookupWebhook., must be bigger than or equal to 36.');
        }
        if ((!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/", $lookup_id))) {
            throw new \InvalidArgumentException("invalid value for \$lookup_id when calling DataNumberLookupWebhook., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.");
        }
    ...
    public function setCountryIso($country_iso)
    {
        if ((mb_strlen($country_iso) > 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling DataNumberLookupWebhook., must be smaller than or equal to 2.');
        }
        if ((mb_strlen($country_iso) < 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling DataNumberLookupWebhook., must be bigger than or equal to 2.');
        }
