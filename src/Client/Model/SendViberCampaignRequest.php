...
/**
 * SendViberCampaignRequest Class Doc Comment
 *
 * @category Class
 * @package  Smscx
 * @author   SMS Connexion
 ...
 */
class SendViberCampaignRequest implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $clientModelName = 'SendViberCampaignRequest';
    ...
    */
    ...
    public function setFrom($from)
    {
        if ((mb_strlen($from) > 15)) {
            throw new \InvalidArgumentException('invalid length for $from when calling SendViberCampaignRequest., must be smaller than or equal to 15.');
        }
        if ((mb_strlen($from) < 3)) {
            throw new \InvalidArgumentException('invalid length for $from when calling SendViberCampaignRequest., must be bigger than or equal to 3.');
        }
    ...
    public function setIdempotencyId($idempotency_id)
    {
        if (!is_null($idempotency_id) && (mb_strlen($idempotency_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $idempotency_id when calling SendViberCampaignRequest., must be smaller than or equal to 36.');
        }
        if (!is_null($idempotency_id) && (mb_strlen($idempotency_id) < 36)) {
            throw new \InvalidArgumentException('invalid length for $idempotency_id when calling SendViberCampaignRequest., must be bigger than or equal to 36.');
        }
        if (!is_null($idempotency_id) && (!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/", $idempotency_id))) {
            throw new \InvalidArgumentException("invalid value for \$idempotency_id when calling SendViberCampaignRequest., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.");
        }
