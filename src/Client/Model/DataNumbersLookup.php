...
/**
 * DataNumbersLookup Class Doc Comment
 *
 * @category Class
 * @package  Smscx
 * @author   SMS Connexion
 ...
 */
class DataNumbersLookup implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $clientModelName = 'DataNumbersLookup';
    ...
    */
    ...
    public function setCountryIso($country_iso)
    {
        if ((mb_strlen($country_iso) > 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling DataNumbersLookup., must be smaller than or equal to 2.');
        }
        if ((mb_strlen($country_iso) < 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling DataNumbersLookup., must be bigger than or equal to 2.');
        }
    ...
    public function setLookupId($lookup_id)
    {
        if ((mb_strlen($lookup_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $lookup_id when calling DataNumbersLookup., must be smaller than or equal to 36.');
        }
        if ((mb_strlen($lookup_id) < 36)) {
            throw new \InvalidArgumentException('invalid length for $lookup_id when calling DataNumbersLookup., must be bigger than or equal to 36.');
        }
        if ((!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/", $lookup_id))) {
            throw new \InvalidArgumentException("invalid value for \$lookup_id when calling DataNumbersLookup., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.");
        }
