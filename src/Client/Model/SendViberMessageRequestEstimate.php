...
/**
 * SendViberMessageRequestEstimate Class Doc Comment
 *
 * @category Class
 * @package  Smscx
 * @author   SMS Connexion
 ...
 */
class SendViberMessageRequestEstimate implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $clientModelName = 'SendViberMessageRequestEstimate';
    ...
    */
    ...
    public function setFrom($from)
    {
        if ((mb_strlen($from) > 15)) {
            throw new \InvalidArgumentException('invalid length for $from when calling SendViberMessageRequestEstimate., must be smaller than or equal to 15.');
        }
        if ((mb_strlen($from) < 3)) {
            throw new \InvalidArgumentException('invalid length for $from when calling SendViberMessageRequestEstimate., must be bigger than or equal to 3.');
        }
    ...
    public function setCountryIso($country_iso)
    {
        if (!is_null($country_iso) && (mb_strlen($country_iso) > 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling SendViberMessageRequestEstimate., must be smaller than or equal to 2.');
        }
        if (!is_null($country_iso) && (mb_strlen($country_iso) < 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling SendViberMessageRequestEstimate., must be bigger than or equal to 2.');
        }
    ...
    public function setTrackData($track_data)
    {
        if (!is_null($track_data) && (mb_strlen($track_data) > 36)) {
            throw new \InvalidArgumentException('invalid length for $track_data when calling SendViberMessageRequestEstimate., must be smaller than or equal to 36.');
        }
        if (!is_null($track_data) && (mb_strlen($track_data) < 36)) {
            throw new \InvalidArgumentException('invalid length for $track_data when calling SendViberMessageRequestEstimate., must be bigger than or equal to 36.');
        }
        if (!is_null($track_data) && (!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/", $track_data))) {
            throw new \InvalidArgumentException("invalid value for \$track_data when calling SendViberMessageRequestEstimate., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.");
        }
