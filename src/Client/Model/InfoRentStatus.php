...
/**
 * InfoRentStatus Class Doc Comment
 *
 * @category Class
 * @package  Smscx
 * @author   SMS Connexion
 ...
 */
class InfoRentStatus implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $clientModelName = 'InfoRentStatus';
    ...
    */
    ...
    public function setRentId($rent_id)
    {
        if ((mb_strlen($rent_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $rent_id when calling InfoRentStatus., must be smaller than or equal to 36.');
        }
        if ((mb_strlen($rent_id) < 36)) {
            throw new \InvalidArgumentException('invalid length for $rent_id when calling InfoRentStatus., must be bigger than or equal to 36.');
        }
        if ((!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/", $rent_id))) {
            throw new \InvalidArgumentException("invalid value for \$rent_id when calling InfoRentStatus., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.");
        }
    ...
    public function setNumberId($number_id)
    {
        if ((mb_strlen($number_id) > 36)) {
            throw new \InvalidArgumentException('invalid length for $number_id when calling InfoRentStatus., must be smaller than or equal to 36.');
        }
        if ((mb_strlen($number_id) < 36)) {
            throw new \InvalidArgumentException('invalid length for $number_id when calling InfoRentStatus., must be bigger than or equal to 36.');
        }
        if ((!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/", $number_id))) {
            throw new \InvalidArgumentException("invalid value for \$number_id when calling InfoRentStatus., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/.");
        }
    ...
    public function setCountryIso($country_iso)
    {
        if ((mb_strlen($country_iso) > 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling InfoRentStatus., must be smaller than or equal to 2.');
        }
        if ((mb_strlen($country_iso) < 2)) {
            throw new \InvalidArgumentException('invalid length for $country_iso when calling InfoRentStatus., must be bigger than or equal to 2.');
        }
     ...
     * Sets callback_url
     *
     * @param string $callback_url Callback URL (or webhook) to get the SMS received on the rented phone number (inbound SMS)
     *
     * @return self
     */
